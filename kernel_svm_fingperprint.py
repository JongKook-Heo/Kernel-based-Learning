...
import warnings
from utils import read_moleculenet_smiles, scaffold_split, smiles_to_df_with_fingerprints
...
def get_parser():
    parser = argparse.ArgumentParser(description= 'Kernel SVM with Molecular Fingerprints')
...
def main(args):
    ...
    train_df = smiles_to_df_with_fingerprints(train_smiles, train_labels)
    val_df = smiles_to_df_with_fingerprints(val_smiles, val_labels)
    test_df = smiles_to_df_with_fingerprints(test_smiles, test_labels)
    ...
    records['Representation'] = 'Fingerprints'
